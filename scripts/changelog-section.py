...
version has no section, letting the caller fall back to a generic note.

Kept deliberately dependency-free (stdlib only) and cross-platform: the Publish
workflow's release job runs it on the runner, but it is equally runnable locally
to preview notes before a tag push.
"""
...
def main() -> int:
    ...
    for i, h in enumerate(heads):
        if h.group(1) == ver:
            ...
            # Drop trailing link-reference lines like `[0.16.6]: https://…/compare/…`.
