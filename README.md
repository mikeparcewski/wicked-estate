...
memory + knowledge layer absorbed 2026-08 — see the note above). SQLite by default; no
...
|---|---|---|---|
| **Equip** | **`wicked-estate`** (this repo) | Your live technical environment, queryable — requirements↔implementation, blast-radius, infra + policy relationships, operational history — plus memory + knowledge with provenance. | Rust · crates.io |
...
the context for this symbol" — across **114 languages** (103 wired tree-sitter grammars + 6
grammar-less mainframe extractors; including legacy enterprise stacks:
VB6/VBA/VBScript/VB.NET, RPG, Delphi, ColdFusion, Progress ABL, PowerBuilder, Visual FoxPro,
LotusScript, Informix 4GL, Crystal Reports) and a mainframe/IaC **estate** layer
...
Two binaries, both on crates.io: the **`wicked-estate` CLI** (index / query / blast-radius /
rank / drift / plugins / …) and the **`wicked-estate-mcp` server** your agent connects to.
Install both:
...
- **Live technical environment, queryable** — the lead: **requirements↔implementation**,
  **blast-radius of change**, **infra + policy relationships**, and **operational history**, all keyed
  ...
  2026-08; agent surface: wicked-garden's `mem`/`search` domains); estate maps what a change
  touches, what protects it, and what it was.
- **Extraction engine — 114 languages (tree-sitter + grammar-less)** — symbols, calls, imports,
  heritage feed the environment above. Languages are **data** (a manifest row + a `.scm` query) —
  adding one is zero core change; 103 of the 114 manifest rows have wired tree-sitter grammars
  (see the generated [coverage matrix](./docs/language-coverage-matrix.md)).
...
- **Layered resolution** — name / scoped / import-map / relative-import → SCIP (precise). Two-phase
  EXTRACT → RESOLVE; resolution is swappable without re-parsing. The on-demand **LSP client is
  built as a library**; its intent-routed edit-plane consumer is designed, not yet wired
  ([ADR-009](./docs/adr/ADR-009-intent-routed-lsp.md)).
...
| `wicked-estate-resolve` | reference resolvers (name/scoped/import-map/relative-import/SCIP/estate) + the LSP client library (ADR-009) |
...
| `wicked-estate-observe` | OTLP HTTP/JSON exporter (`OtlpSink`) + wired emission sites |
...
- **External DB + OpenTelemetry**: the **Postgres** backend and the **OTLP** exporter are **built** ✅
  ...
  isn't yet benchmarked at scale vs. SQLite, OTel has no zero-to-dashboard guide yet, and the
  SurrealDB challenger (built behind `--features surrealdb`) has no `open_store` factory wiring
  and no bake-off verdict.
...
plugins) · **[docs/MIGRATION-0.15.md](./docs/MIGRATION-0.15.md) (upgrading a 0.14.x DB — the
mandatory re-index runbook)** · [docs/ENGINE-CONTRACT.md](./docs/ENGINE-CONTRACT.md)
(invariants) · [docs/agent-behavior-rules.md](./docs/agent-behavior-rules.md) (the runtime contract)
· [docs/adr/](./docs/adr/) (decisions; [ADR-009](./docs/adr/ADR-009-intent-routed-lsp.md) defines
the intent-routed LSP edit plane) · [docs/plan/WAVE-PLAN.md](./docs/plan/WAVE-PLAN.md) (tracker)
