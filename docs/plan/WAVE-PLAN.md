...
| W3 | Precise resolution core | ◑ **W3.1 RETIRED 2026-08-28** (`MethodResolutionSynthesizer` deleted — emit set ⊂ `ScopedNameResolver`, never in a production slice; ADR-007 superseding note); **W3.3 ✅** (`wicked-estate-resolve/src/lsp.rs`, on-demand LSP client, JSON-RPC stdio, servers not bulk-invoked; on-demand consumer = W3.6 follow-up); **W3.4 ✅** (confidence calibration, `resolve_all` max-confidence dedup); **W3.5 RETIRED 2026-08-28** (the precision dashboard never existed in bench; `measure_synth_precision`/`SYNTH_PRECISION_FLOOR` deleted — ADR-007 superseding note); **W3.2 SUPERSEDED-BY-SCIP** (ADR-007: TSG port not needed for SCIP-covered languages; remains future option for non-SCIP langs) | 2 ✅ / 2 retired / 1 superseded |
...
**Superseded:** W3.2 TSG port — the SCIP precise tier (W2.2) + `ScopedNameResolver` cover all originally targeted languages at equal or higher precision (`MethodResolutionSynthesizer`, once listed here, was retired 2026-08-28 — it never added an edge; ADR-007 superseding note). TSG remains a future option for non-SCIP languages (ADR-007).
...
- [x] **W3.1** **`MethodResolutionSynthesizer`** — **RETIRED 2026-08-28** (ADR-007 superseding note): its emit set was a strict subset of `ScopedNameResolver`'s Calls path at lower confidence, so it could never add an edge, and it was never in any production slice. Struct, precision monitor, and tests deleted; the structural theorem is pinned by `slice_plus_unique_callable_heuristic_adds_no_edge`.
...
- [x] **W3.3** **On-demand LSP tier** — `crates/wicked-estate-resolve/src/lsp.rs`: minimal JSON-RPC 2.0 stdio client; `ServerRegistry` maps language→command; per-request **deadline** timeout (frame-pump reader thread + mpsc — notification chatter never restarts the clock; default 10s, injectable); `LspTier` + `LspClient`. ON DEMAND only — never invoked during bulk indexing. **AC:** ✅ LSP client drives `initialize` + `textDocument/definition` (`tests/lsp_live.rs`, probe-and-skip); servers not spawned in batch. A client library by design — no `Resolver` impl, no edge emission; the consumer is **W3.6**.
- [x] **W3.4** **Confidence calibration** — tier ordering: Tags(0.3) < Heuristic(0.5) < ImportMap(0.60–0.65) < Tsg(0.8) < Scip/Lsp(1.0). `resolve_all` keeps max-confidence edge per `(source, target, kind)` triple. **AC:** ✅ provenance trail on every edge; dedup verified by tests.
- [x] **W3.5** **Resolution precision dashboard** — **RETIRED 2026-08-28** (ADR-007 superseding note): the "tracked in wicked-estate-bench" claim was false (zero bench references); `measure_synth_precision`/`SynthPrecision`/`SYNTH_PRECISION_FLOOR` had no subject once W3.1 was retired and were deleted with it.
- [ ] **W3.6** **On-demand LSP consumer** — designed in **`docs/adr/ADR-009-intent-routed-lsp.md`** (intent-routed planes: EDIT = position-anchored Definition/References/Hover MCP tools + CLI twins, LSP-first with `LSP-FALLBACK:` warn-and-serve; UNDERSTAND = graph only, LSP never consulted; capability report; installer skill as a pointer-model `skill://` resource; registry→data promotion). Phase-0 (transport deadline timeout + didOpen, landed) changed no activation. LOCKED: on-demand only, never bulk. **AC:** a caller reaches `LspTier::definition`/`references` from the MCP or CLI surface; `lsp.rs` stays out of every bulk resolver slice; plus the ADR-009 §5 implementation AC list (conformance count + goldens, cache exclusion, doc-count updates, References cap + `LSP-TRUNCATED`, required-root parameter, §3.1 notes-cell-only update).
