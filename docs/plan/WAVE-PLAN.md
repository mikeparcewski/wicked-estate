...
- [ ] **W3.6** **On-demand LSP consumer** — the designed-but-unwired consumer for `lsp.rs` (W3.3): an MCP/CLI single-symbol definition/references tool carrying the `resolve.lsp` span. LOCKED: on-demand only, never bulk. **AC:** a caller reaches `LspTier::definition`/`references` from the MCP or CLI surface; `lsp.rs` stays out of every bulk resolver slice.
