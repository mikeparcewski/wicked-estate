...
fn sc009_all_four_stores_open_and_all_reads_return_nonempty() {
    ...
    // ── tools/list with all 4 stores open → exactly 29 tools ─────────────────
    // 11 estate (unconditional, incl. rules.recall) + 7 memory + 7 knowledge + 4 proposal = 29.
    ...
    assert_eq!(
        tools.len(),
        29,
        "tools/list with all 4 stores must return exactly 29 tools \
         (11 estate + 7 memory + 7 knowledge + 4 proposal); got {}: {:?}",
        tools.len(),
        tools
            .iter()
