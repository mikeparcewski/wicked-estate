...
fn memory_tool_schemas() -> Vec<Value> {
    vec![
        json!({"name":"memory.capture","description":"Capture a new memory node (episodic/semantic/procedural/archival).","inputSchema":{"type":"object","required":["content"],"properties":{"content":{"type":"string"},"kind":{"type":"string","enum":["working","episode","entity","fact","skill","archive"]},"tier":{"type":"string","enum":["working","episodic","semantic","procedural","archival"]},"scope":{"type":"string","description":"Hierarchical scope path: slash-separated kind:id segments (e.g. \"org:acme/agent:claude\"). Empty, omitted, or null = root. Segments that are not kind:id pairs are rejected (invalid params), never silently dropped."},"about":{"type":"array","items":{"type":"string"}},"facets":{"type":"object","additionalProperties":{"type":"string"},"description":"Orthogonal intent-matching facets tagged to this memory's natural axis (e.g. {\"cli\":\"codex\"} or {\"repo\":\"wicked-estate\"}). Axis must match ^[a-z][a-z0-9_-]*$, value non-empty; invalid ⇒ invalid params."}}}}),
        json!({"name":"memory.recall","description":"Conversational recall: token-budgeted slice relevant to a query in scope. Each item carries memory_id, scope (the item's own hierarchical scope, e.g. org:acme/agent:claude), content, tier, and score. Default visibility is inheritance: memories at `scope` or an ANCESTOR of it. Pass scope_prefix to flip to subtree-inclusive visibility instead.","inputSchema":{"type":"object","required":["query"],"properties":{"query":{"type":"string"},"scope":{"type":"string"},"scope_prefix":{"type":"string","description":"Optional subtree filter: when present, candidates are memories whose scope equals this prefix or DESCENDS from it (same predicate as memory.erase/memory.coverage scope_prefix), REPLACING the ancestor-visible inheritance filter from `scope`. \"\" = root subtree = every memory. Omitted or null = inheritance behavior. A recall with a prefix previews exactly the set memory.erase with that prefix would delete."},"seeds":{"type":"array","items":{"type":"string"}},"token_budget":{"type":"integer","default":2000},"intent":{"type":"object","additionalProperties":{"type":"string"},"description":"The session's intent tuple (axis→value). Recall returns the UNION of memories whose facets are a subset of this intent, specificity-ranked. A memory constraining an axis absent from intent is excluded. Omitted ⇒ empty ⇒ only unfaceted memories surface."}}}}),
