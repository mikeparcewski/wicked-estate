...
impl XedgeStore {
    ...
    /// Delete all xedge rows where `src_engine = ?1 AND src_id = ?2`.
    /// Called by MemoryApi::erase to clean up cross-engine links.
    pub fn delete_by_src_id(&self, src_engine: &str, src_id: &str) -> Result<u64> {
        let conn = self.conn.lock().expect("xedge mutex poisoned");
        let n = conn
            .execute(
                "DELETE FROM xedges WHERE src_engine = ?1 AND src_id = ?2",
                rusqlite::params![src_engine, src_id],
            )
            .map_err(map_err)?;
        Ok(n as u64)
    }
